...
> **The MAVLink encoder is checked against bytes a real vehicle sent, and that caught a bug the round trip could not.** Every one of the 1058 recorded SITL frames is decoded, re-encoded from its parts, and required to match the original bytes exactly. The first implementation truncated v2 trailing zeros all the way to nothing — the obvious reading of the rule, and wrong: the recording contains **no** zero-length payload and eight frames carrying a single zero byte, so truncation floors at one. Encoding and decoding through this repo's own code agreed perfectly either way, because the decoder reconstructs the zeros. Only the real bytes disagreed.
